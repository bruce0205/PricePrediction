...
object RunGBT {
  ...
  def evaluateModel(model: GradientBoostedTreesModel, validationData: RDD[LabeledPoint]): (Double, Double) = {
  ...
  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numIterationsArray: Array[Int], lossArray: Array[String], impurityArray: Array[String], maxDepthArray: Array[Int], maxBinsArray: Array[Int], learningRateArray: Array[Double]): GradientBoostedTreesModel = {
    val evaluationsArray =
      for (numIterations <- numIterationsArray; loss <- lossArray; impurity <- impurityArray; maxDepth <- maxDepthArray; maxBins <- maxBinsArray; learningRate <- learningRateArray) yield {
        ...
        println("參數組合：numIterations:" + numIterations + ", loss:" + loss + ", impurity:" + impurity + ", maxDepth:" + maxDepth + ", maxBins:" + maxBins + ", learningRate:" + learningRate + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time)
        (numIterations, loss, impurity, maxDepth, maxBins, learningRate, RMSE, time)
      }
    ...
    println("調校後最佳參數：numIterations:" + BestEval._1 + ", loss:" + BestEval._2 + ", impurity:" + BestEval._3 + ", maxDepth:" + BestEval._4 + ", maxBins:" + BestEval._5 + ", learningRate:" + BestEval._6 + ", RMSE:" + Utility.decimalRoundOff(BestEval._7, 2) + ", timeSpent:" + BestEval._8)
