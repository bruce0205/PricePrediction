...
object RunRidgeRegression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Run Ridge Regression end....")
  }
  ...
  def PredictData(sc: SparkContext, model: RidgeRegressionModel, predictData: RDD[LabeledPoint], predictCnt: Int): Unit = {
  ...
  def trainModel(trainData: RDD[LabeledPoint], numIterations: Int, stepSize: Double, regParam: Double, minBatchFraction: Double): (RidgeRegressionModel, Double) = {
    val startTime = new DateTime()
    val model: RidgeRegressionModel = RidgeRegressionWithSGD.train(trainData, numIterations, stepSize, regParam, minBatchFraction)
    val endTime = new DateTime()
    val duration = new Duration(startTime, endTime)
    (model, duration.getMillis())
  }

  def evaluateModel(model: RidgeRegressionModel, validationData: RDD[LabeledPoint]): (Double, Double) = {
  ...
  def parametersTuning(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint]): RidgeRegressionModel = {
  ...
  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numIterationsArray: Array[Int], stepSizeArray: Array[Double], regParamArray: Array[Double], minBatchFractionArray: Array[Double]): RidgeRegressionModel = {
    val evaluationsArray =
      for (numIterations <- numIterationsArray; stepSize <- stepSizeArray; regParam <- regParamArray; minBatchFraction <- minBatchFractionArray) yield {
        ...
        // println("參數組合：numIterations:" + numIterations + ", stepSize:" + stepSize + ", regParam:" + regParam + ", minBatchFraction:" + minBatchFraction + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time)
        (numIterations, stepSize, regParam, minBatchFraction, RMSE, time)
      }
    ...
    println("調校後最佳參數：numIterations:" + BestEval._1 + ", stepSize:" + BestEval._2 + ", regParam:" + BestEval._3 + ", minBatchFraction:" + BestEval._4 + ", RMSE:" + Utility.decimalRoundOff(BestEval._6, 2) + ", timeSpent:" + BestEval._6)
