...
object RunLinearReregression {
  ...
  def evaluateModel(model: LinearRegressionModel, validationData: RDD[LabeledPoint]): (Double, Double) = {
  ...
  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numIterationsArray: Array[Int], stepSizeArray: Array[Double], minBatchFractionArray: Array[Double]): LinearRegressionModel = {
    val evaluationsArray =
      for (numIterations <- numIterationsArray; stepSize <- stepSizeArray; minBatchFraction <- minBatchFractionArray) yield {
        ...
        // println("參數組合：numIterations:" + numIterations + ", stepSize:" + stepSize + ", minBatchFraction:" + minBatchFraction + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time)
        (numIterations, stepSize, minBatchFraction, RMSE, time)
      }
    ...
    println("調校後最佳參數：numIterations:" + BestEval._1 + ", stepSize:" + BestEval._2 + ", minBatchFraction:" + BestEval._3 + ", RMSE:" + Utility.decimalRoundOff(BestEval._4, 2) + ", timeSpent:" + BestEval._5)
