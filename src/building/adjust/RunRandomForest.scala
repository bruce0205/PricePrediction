...
object RunRandomForest {
  ...
  def evaluateModel(model: RandomForestModel, validationData: RDD[LabeledPoint]): (Double, Double) = {
  ...
  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numTreesArray: Array[Int], featureSubsetStrategyArray: Array[String], impurityArray: Array[String], maxDepthArray: Array[Int], maxBinsArray: Array[Int]): RandomForestModel = {
    val evaluationsArray =
      for (numTrees <- numTreesArray; featureSubsetStrategy <- featureSubsetStrategyArray; impurity <- impurityArray; maxDepth <- maxDepthArray; maxBins <- maxBinsArray) yield {
        ...
        // println("參數組合：numTrees:" + numTrees + ", featureSubsetStrategy:" + featureSubsetStrategy + ", impurity:" + impurity + ", maxDepth:" + maxDepth + ", maxBins:" + maxBins + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time)
    ...
    println("調校後最佳參數：numTrees:" + BestEval._1 + ", featureSubsetStrategy:" + BestEval._2 + ", impurity:" + BestEval._3 + ", maxDepth:" + BestEval._4 + ", maxBins:" + BestEval._5 + ", RMSE:" + Utility.decimalRoundOff(BestEval._6, 2) + ", timeSpent:" + BestEval._7)
