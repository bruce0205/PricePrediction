...
object RunLinearReregression_mape {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Run Linear Regression end....")
  }
  ...
  def PredictData(sc: SparkContext, model: LinearRegressionModel, predictData: RDD[LabeledPoint], predictCnt: Int): Unit = {
  ...
  def trainModel(trainData: RDD[LabeledPoint], numIterations: Int, stepSize: Double, minBatchFraction: Double): (LinearRegressionModel, Double) = {
    val startTime = new DateTime()
    val model: LinearRegressionModel = LinearRegressionWithSGD.train(trainData, numIterations, stepSize, minBatchFraction)
    val endTime = new DateTime()
    val duration = new Duration(startTime, endTime)
    (model, duration.getMillis())
  }

  def evaluateModel(model: LinearRegressionModel, validationData: RDD[LabeledPoint]): (Double, Double, Double) = {
  ...
  def parametersTuning(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint]): LinearRegressionModel = {
    ...
    println("")
    */
    val bestModel = evaluateAllParameter(trainData, validationData, numIterationsArray, stepSizeArray, minBatchFractionArray)

    return (bestModel)
  }


  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], numIterationsArray: Array[Int], stepSizeArray: Array[Double], minBatchFractionArray: Array[Double]): LinearRegressionModel = {
    val evaluationsArray =
      for (numIterations <- numIterationsArray; stepSize <- stepSizeArray; minBatchFraction <- minBatchFractionArray) yield {
        val (model, time) = trainModel(trainData, numIterations, stepSize, minBatchFraction)
        ...
        // println("參數組合：numIterations:" + numIterations + ", stepSize:" + stepSize + ", minBatchFraction:" + minBatchFraction + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time + ", MAE:" + MAE + ", MAPE:" + MAPE)
        (numIterations, stepSize, minBatchFraction, MAPE, time)
      }
    ...
    println("調校後最佳參數：numIterations:" + BestEval._1 + ", stepSize:" + BestEval._2 + ", minBatchFraction:" + BestEval._3 + ", MAPE:" + Utility.decimalRoundOff(BestEval._4, 2) + ", timeSpent:" + BestEval._5)
