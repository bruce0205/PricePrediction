...
object RunCART_mape {
  ...
  def evaluateModel(model: DecisionTreeModel, validationData: RDD[LabeledPoint]): (Double, Double, Double) = {
  ...
  def evaluateAllParameter(trainData: RDD[LabeledPoint], validationData: RDD[LabeledPoint], impurityArray: Array[String], maxDepthArray: Array[Int], maxBinsArray: Array[Int]): DecisionTreeModel = {
    val evaluationsArray =
      for (impurity <- impurityArray; maxDepth <- maxDepthArray; maxBins <- maxBinsArray) yield {
        ...
        // println("參數組合：impurity:" + impurity + ", maxDepth:" + maxDepth + ", maxBins:" + maxBins + ", RMSE:" + Utility.decimalRoundOff(RMSE, 2) + ", timeSpent:" + time + ", MAE:" + MAE + ", MAPE:" + MAPE)
        (impurity, maxDepth, maxBins, MAPE, time)
      }
    ...
    println("調校後最佳參數：impurity:" + BestEval._1 + ", maxDepth:" + BestEval._2 + ", maxBins:" + BestEval._3 + ", MAPE:" + Utility.decimalRoundOff(BestEval._4, 2) + ", timeSpent:" + BestEval._5)
